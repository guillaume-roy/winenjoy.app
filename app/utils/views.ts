export class Views {
    public static main: string = "views/main/main";
    public static editTasting: string = "views/edit-tasting/edit-tasting";
    public static viewTasting: string = "views/view-tasting/view-tasting";
    public static gradientColorPickerModal: string = "views/gradient-color-picker-modal/gradient-color-picker-modal";
    public static aromasPicker: string = "views/aromas-picker/aromas-picker";
}
